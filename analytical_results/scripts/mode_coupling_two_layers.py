...
from src.utils.vis_style import EG_COLOR, TARGET_COLOR, _set_pub_style
...
from src.inits.init_two_layer import build_init as build_init_two
...
def omega_from_W_and_Wdot(W, Wdot, tiny=1e-10, degeneracy_safe=True):
    U, s, Vt = svd(W, full_matrices=False); V = Vt.T
    K = len(s)
...
def svd_and_omega_history(W_series, degeneracy_safe=True):
    W_series = np.asarray(W_series)
    ...
    for t in range(T):
        _, s, _, Omega = omega_from_W_and_Wdot(W_series[t], Wdot[t], degeneracy_safe=degeneracy_safe)
        S[t, :len(s)] = s[:K]
        Omega_hist[t, :len(s), :len(s)] = Omega[:K,:K]
    return S, Omega_hist
...
def plot_singulars(ax, t, S, s_targets=None):
    ...
    ax.set_xlabel("Training step"); ax.set_ylabel("Singular values")
...
def plot_coupling_and_heatmap(ax_curves, ax_heat, t, Omega_hist, pairs=None, aggregate=False):
    from src.utils.vis_style import EG_COLOR, _set_pub_style
...
def parse_args():
    ap = argparse.ArgumentParser(description="Mode coupling (two-layer).")
    ...
    defaults = dict(
        algo="eg", P=800, N1=8, N2=16, N3=8, noise_std=0.02,
        eta=0.1, steps=1200, record_every=20, seed=0,
        pairs=None, aggregate=False, degeneracy_safe=True,
        outdir="results/mode_coupling_two", outfile="two_layer_mode_coupling.pdf",
        save_npz=None,
    )
...
def main():
    ...
    W21_0, W32_0 = build_init_two("gauss-small", Sigma_yx, cfg["N1"], cfg["N2"], cfg["N3"],
                                  seed=cfg["seed"], scale=1e-2)

    if cfg["algo"] == "eg":
        t, W_hist = simulate_EG_two_layer(W21_0, W32_0, Sigma_x, Sigma_yx,
                                          eta=cfg["eta"], n_steps=cfg["steps"], record_every=cfg["record_every"])
    else:
        t, W_hist = simulate_GD_two_layer(W21_0, W32_0, Sigma_x, Sigma_yx,
                                          eta=cfg["eta"], n_steps=cfg["steps"], record_every=cfg["record_every"])

    N1, N2, N3 = cfg["N1"], cfg["N2"], cfg["N3"]
    Wprod = get_Wprod_hist(W_hist, (N1, N2, N3)) 

    S, Omega = svd_and_omega_history(Wprod, degeneracy_safe=cfg["degeneracy_safe"])
    ...
    _ = plot_coupling_and_heatmap(axc, axh, t, Omega, pairs=cfg["pairs"], aggregate=cfg["aggregate"])
    fig2.tight_layout()
